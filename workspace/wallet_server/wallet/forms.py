...
class SignUpForm(FlaskForm):
    ...
    user_id = StringField(
        '사용자 아이디',
        validators=[DataRequired('사용자 아이디는 반드시 입력해야합니다.')]
    )
    ...
    name = StringField(
        '이름(성 포함)',
        validators=[DataRequired('이름은 필수입니다.'), Length(min=2, max=25)]
    )
    phone_mobile = StringField(
        '휴대전화 번호',
        validators=[DataRequired('휴대전화 번호는 필수사항입니다. 향후 차가 인증을 위해 사용됩니다.')]
    )
    email = EmailField(
        '이메일',
        validators=[DataRequired('이메일은 필수입력입니다.'), Email() ]
    )
...
class TransferForm(FlaskForm):
    amount = FloatField(
        label = '이체 수량',
        validators=[
            DataRequired('코인 이체 슈량은 필수입력 사항입니다.'),
    ...
    recv_addr = StringField(
        label='받을사람 지갑 주소',
        validators=[DataRequired('받는 사람 지갑주소는 필수입니다.')]
    )
